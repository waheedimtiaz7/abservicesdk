...
class ResponseBusinessEntity202001 extends ResponseBusinessEntity201408
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \AbnServiceSDK\StructType\AwefEntity[]
     */
     ...
     * @param \AbnServiceSDK\StructType\AwefEntity[] $awefEntity
     * @param \AbnServiceSDK\StructType\AwefEntity[] $awefOperator
     */
     ...
     * @return \AbnServiceSDK\StructType\AwefEntity[]
     */
    ...
    public static function validateAwefEntityForArrayConstraintFromSetAwefEntity(?array $values = []): string
    {
        ...
        foreach ($values as $responseBusinessEntity202001AwefEntityItem) {
            ...
            if (!$responseBusinessEntity202001AwefEntityItem instanceof \AbnServiceSDK\StructType\AwefEntity) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The awefEntity property can only contain items of type \AbnServiceSDK\StructType\AwefEntity, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \AbnServiceSDK\StructType\AwefEntity[] $awefEntity
     * @return \AbnServiceSDK\StructType\ResponseBusinessEntity202001
     */
     ...
     * @throws InvalidArgumentException
     * @param \AbnServiceSDK\StructType\AwefEntity $item
     * @return \AbnServiceSDK\StructType\ResponseBusinessEntity202001
     */
    public function addToAwefEntity(\AbnServiceSDK\StructType\AwefEntity $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \AbnServiceSDK\StructType\AwefEntity) {
            throw new InvalidArgumentException(sprintf('The awefEntity property can only contain items of type \AbnServiceSDK\StructType\AwefEntity, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
    ...
    public static function validateAwefOperatorForArrayConstraintFromSetAwefOperator(?array $values = []): string
    {
        ...
        foreach ($values as $responseBusinessEntity202001AwefOperatorItem) {
            // validation for constraint: itemType
            if (!$responseBusinessEntity202001AwefOperatorItem instanceof \AbnServiceSDK\StructType\AwefEntity) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The awefOperator property can only contain items of type \AbnServiceSDK\StructType\AwefEntity, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \AbnServiceSDK\StructType\ResponseBusinessEntity202001
     */
    public function addToAwefOperator(\AbnServiceSDK\StructType\AwefEntity $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \AbnServiceSDK\StructType\AwefEntity) {
            throw new InvalidArgumentException(sprintf('The awefOperator property can only contain items of type \AbnServiceSDK\StructType\AwefEntity, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
