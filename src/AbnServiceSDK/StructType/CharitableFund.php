...
class CharitableFund extends AbstractStructBase
{
     ...
     * @param \AbnServiceSDK\StructType\OrganisationSimpleName[] $pbiName
     */
    ...
    /**
     * Get pbiName value
     * @return \AbnServiceSDK\StructType\OrganisationSimpleName[]
     */
    ...
    public static function validatePbiNameForArrayConstraintFromSetPbiName(?array $values = []): string
    {
        ...
        foreach ($values as $charitableFundPbiNameItem) {
            ...
            if (!$charitableFundPbiNameItem instanceof \AbnServiceSDK\StructType\OrganisationSimpleName) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The pbiName property can only contain items of type \AbnServiceSDK\StructType\OrganisationSimpleName, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \AbnServiceSDK\StructType\CharitableFund
     */
    public function addToPbiName(\AbnServiceSDK\StructType\OrganisationSimpleName $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \AbnServiceSDK\StructType\OrganisationSimpleName) {
            throw new InvalidArgumentException(sprintf('The pbiName property can only contain items of type \AbnServiceSDK\StructType\OrganisationSimpleName, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->pbiName[] = $item;
