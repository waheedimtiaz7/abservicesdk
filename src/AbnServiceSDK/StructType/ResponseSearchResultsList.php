...
class ResponseSearchResultsList extends ResponseBody
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \AbnServiceSDK\StructType\SearchResultsRecord[]
     */
     ...
     * @param \AbnServiceSDK\StructType\SearchResultsRecord[] $searchResultsRecord
     */
     ...
     * @param int $numberOfRecords
     * @return \AbnServiceSDK\StructType\ResponseSearchResultsList
     */
    ...
    public static function validateSearchResultsRecordForArrayConstraintFromSetSearchResultsRecord(?array $values = []): string
    {
        ...
        foreach ($values as $responseSearchResultsListSearchResultsRecordItem) {
            ...
            if (!$responseSearchResultsListSearchResultsRecordItem instanceof \AbnServiceSDK\StructType\SearchResultsRecord) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The searchResultsRecord property can only contain items of type \AbnServiceSDK\StructType\SearchResultsRecord, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \AbnServiceSDK\StructType\SearchResultsRecord $item
     * @return \AbnServiceSDK\StructType\ResponseSearchResultsList
     */
    public function addToSearchResultsRecord(\AbnServiceSDK\StructType\SearchResultsRecord $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \AbnServiceSDK\StructType\SearchResultsRecord) {
            throw new InvalidArgumentException(sprintf('The searchResultsRecord property can only contain items of type \AbnServiceSDK\StructType\SearchResultsRecord, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
